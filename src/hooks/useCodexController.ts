...
function buildImportNotice(
	...
	const suffix =
		failureCount > 0 && firstFailure ? notices.importSummaryFirstFailure(firstFailure.source, firstFailure.error) : '';
...
function buildRemoteProxyFallback(server: RemoteServerConfig, lastError: string): RemoteProxyStatus {
	return {
		installed: false,
...
export function useCodexController() {
	...
	const currentCount = useMemo(() => accounts.filter((account) => account.isCurrent).length, [accounts]);
	const sortedAccounts = useMemo(() => sortAccountsByRemaining(accounts), [accounts]);

	const localizeError = useCallback((error: string) => localizeBackendError(error, locale), [locale]);
	...
	const remoteProxyStatuses = useMemo<Record<string, RemoteProxyStatus>>(
		() =>
			Object.fromEntries(
				Object.entries(remoteProxyStatusesRaw).map(([id, status]) => [id, localizeRemoteProxyStatus(status)]),
			),
	...
	}, [addFlow, copy.notices, loadAccounts, localizeError, refreshUsage, restoreAuthAfterAddFlow]);
	...
	}, [addFlow, copy.notices, importingUpload, localizeError, restoreAuthAfterAddFlow, startingAdd]);
	...
	const onImportAuthFiles = useCallback(
		async (items: AuthJsonImportInput[]) => {
			...
			try {
				...
				setNotice({
					type: 'error',
					message: copy.notices.importFailedPlain(copy.notices.fileImportPrefix, localizeError(String(error))),
				});
			} finally {
	...
	const onStartApiProxy = useCallback(
		...
		[apiProxyStatus.running, copy.notices, localizeApiProxyStatus, localizeError, startingApiProxy],
	);
	...
	}, [apiProxyStatus.running, copy.notices, localizeApiProxyStatus, localizeError, stoppingApiProxy]);
	...
	const onDeployRemoteProxy = useCallback(
		async (server: RemoteServerConfig) => {
			...
			if (!(await ensureRemoteLocalDependency(server))) {
				setRemoteDeployProgress((current) => (current?.serverId === server.id ? null : current));
				return;
			}
	...
	const onStartCloudflared = useCallback(
		...
		[cloudflaredStatus.running, copy.notices, localizeCloudflaredStatus, localizeError, startingCloudflared],
	);
	...
	}, [cloudflaredStatus.running, copy.notices, localizeCloudflaredStatus, localizeError, stoppingCloudflared]);
			...
			try {
				...
				if (settings.syncOpencodeOpenaiAuth) {
					if (result.opencodeSyncError) {
						baseNotice = {
							type: 'error',
							message: copy.notices.opencodeSyncFailed(baseNotice.message, localizeError(result.opencodeSyncError)),
						};
				...
				if (settings.restartEditorsOnSwitch) {
					if (result.editorRestartError) {
						baseNotice = {
							type: 'error',
							message: copy.notices.editorRestartFailed(baseNotice.message, localizeError(result.editorRestartError)),
						};
					} else if (result.restartedEditorApps.length > 0) {
						const restartedLabels = result.restartedEditorApps.map((id) => copy.editorAppLabels[id] ?? id).join(' / ');
						baseNotice = {
							...baseNotice,
	...
	const onUpdateRemoteServers = useCallback(
		async (remoteServers: RemoteServerConfig[]) => {
			await updateSettings({ remoteServers }, { silent: true, keepInteractive: true });
		},
		[updateSettings],
	);
