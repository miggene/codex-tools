...
export function isSupportedLocale(value: string | null | undefined): value is AppLocale {
	return value === 'zh-CN' || value === 'en-US' || value === 'ja-JP' || value === 'ko-KR' || value === 'ru-RU';
}
...
function compileLocale(raw: RawMessageCatalog): MessageCatalog {
	return {
		...
		apiProxy: {
			...raw.apiProxy,
			remoteDeployProgressTitle: (label) => fillTemplate(raw.apiProxy.remoteDeployProgressTitle, { label }),
		},
		...
		updateDialog: {
			...
			subtitle: (currentVersion) => fillTemplate(raw.updateDialog.subtitle, { currentVersion }),
		...
		notices: {
			...
			updateDownloadingPercent: (percent) => fillTemplate(raw.notices.updateDownloadingPercent, { percent }),
			...
			openManualDownloadFailed: (error) => fillTemplate(raw.notices.openManualDownloadFailed, { error }),
			addAccountAutoImportFailed: (error) => fillTemplate(raw.notices.addAccountAutoImportFailed, { error }),
			...
			opencodeSyncFailed: (base, error) => fillTemplate(raw.notices.opencodeSyncFailed, { base, error }),
			opencodeSynced: (base) => fillTemplate(raw.notices.opencodeSynced, { base }),
			editorRestartFailed: (base, error) => fillTemplate(raw.notices.editorRestartFailed, { base, error }),
			editorsRestarted: (base, labels) => fillTemplate(raw.notices.editorsRestarted, { base, labels }),
			...
			importFailedPlain: (prefix, error) => fillTemplate(raw.notices.importFailedPlain, { prefix, error }),
			importFailedWithSource: (prefix, source, error) =>
				fillTemplate(raw.notices.importFailedWithSource, { prefix, source, error }),
			importFailedNoValidJson: (prefix) => fillTemplate(raw.notices.importFailedNoValidJson, { prefix }),
			...
			proxyKeyRefreshFailed: (error) => fillTemplate(raw.notices.proxyKeyRefreshFailed, { error }),
			installingDependency: (name) => fillTemplate(raw.notices.installingDependency, { name }),
			dependencyInstalled: (name) => fillTemplate(raw.notices.dependencyInstalled, { name }),
			dependencyInstallFailed: (name, error) => fillTemplate(raw.notices.dependencyInstallFailed, { name, error }),
			remoteStatusFailed: (label, error) => fillTemplate(raw.notices.remoteStatusFailed, { label, error }),
			remoteProxyDeployed: (label) => fillTemplate(raw.notices.remoteProxyDeployed, { label }),
			remoteProxyDeployFailed: (label, error) => fillTemplate(raw.notices.remoteProxyDeployFailed, { label, error }),
			remoteProxyStarted: (label) => fillTemplate(raw.notices.remoteProxyStarted, { label }),
			remoteProxyStartFailed: (label, error) => fillTemplate(raw.notices.remoteProxyStartFailed, { label, error }),
			remoteProxyStopped: (label) => fillTemplate(raw.notices.remoteProxyStopped, { label }),
			remoteProxyStopFailed: (label, error) => fillTemplate(raw.notices.remoteProxyStopFailed, { label, error }),
			remoteLogsFailed: (label, error) => fillTemplate(raw.notices.remoteLogsFailed, { label, error }),
			pickIdentityFileFailed: (error) => fillTemplate(raw.notices.pickIdentityFileFailed, { error }),
			cloudflaredInstallFailed: (error) => fillTemplate(raw.notices.cloudflaredInstallFailed, { error }),
			cloudflaredStarted: (target) => fillTemplate(raw.notices.cloudflaredStarted, { target }),
			cloudflaredStartFailed: (error) => fillTemplate(raw.notices.cloudflaredStartFailed, { error }),
			cloudflaredStopFailed: (error) => fillTemplate(raw.notices.cloudflaredStopFailed, { error }),
		},
	};
}
